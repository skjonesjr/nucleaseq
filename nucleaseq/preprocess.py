...
from seqtools import dna_rev_comp, bases, load_read_name_seq_items, load_sample_given_read_name
from OligosContainer import OligosContainer
...
def preprocess_cut(arguments):
    ...
    def identify_side_and_barcode(seq):
        for cr, side in [(oligo_container.cr_left, 'left'),
            ...
            res = editmeasures.prefix_identification(cr, seq, arguments.max_primer_err)
            if res:
                pos, cr_edits = res
    ...
    last_end = arguments.end or len(read_name_items)
    log.info('Start idx: {:,d}'.format(start))
    log.info('End idx: {:,d}'.format(arguments.end))
    log.info('Increment: {:,d}'.format(arguments.inc))
...
def preprocess_uncut(arguments):
    log.info('NProcs: {}'.format(arguments.nprocs or 'All'))
