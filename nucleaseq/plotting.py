...
def plot_2d_mismatches(sequence, sequence_labels, lower_kKM_matrix, upper_kKM_matrix=None, fontsize=18, cmap='viridis', normalize=False, force_full_bounds=False):
    ...
    ms = add_data(fig, gs[data_index], lower_kKM_matrix, upper_kKM_matrix, normalize=normalize, cmap=cmap, force_full_bounds=force_full_bounds, show_base_legend=True, grid_line_spacing=dimension)
    # Add a color bar to the right side to quantify the colors in the main figure
    add_colorbar(fig, gs[cbar_index], ms, fontsize)
    # color the labels


def plot_position_diff(sequence, sequence_labels, lower_kKM_matrix, upper_kKM_matrix=None, normalize=True, fontsize=18,
                       positions_are_merged=True, colorbar_label='Relative Normalized kKMs ($k_{B}T$)', cmap='RdBu'):
    ...
    # Add data to the main part of the figure
    ms = add_data(fig, gs[data_index], lower_kKM_matrix, upper_kKM_matrix, normalize=normalize, cmap=cmap, force_full_bounds=normalize)
    # Add a color bar to the right side to quantify the colors in the main figure
    add_colorbar(fig, gs[cbar_index], ms, fontsize, label=colorbar_label)


def plot_2d_deletions(sequence, sequence_labels, lower_kKM_matrix, upper_kKM_matrix=None, fontsize=18, cmap='viridis', normalize=False, force_full_bounds=False):
    gs, indexes, (width_ratios, height_ratios) = get_gridspec(sequence, 1)
    data_index, left_seq_index, bottom_seq_index, cbar_index = indexes
    fig = plt.figure(figsize=(sum(width_ratios), sum(height_ratios)))
    # Add the sequence labels to the left of the figure
    ...
    # Add data to the main part of the figure
    ms = add_data(fig, gs[data_index], lower_kKM_matrix, upper_kKM_matrix, cmap=cmap, grid_line_spacing=1, normalize=normalize, force_full_bounds=force_full_bounds)
    # Add a color bar to the right side to quantify the colors in the main figure
    add_colorbar(fig, gs[cbar_index], ms, fontsize)


def plot_complement_stretches(sequence, sequence_labels, lower_kKM_matrix, upper_kKM_matrix=None, fontsize=18, cmap='viridis', normalize=False, force_full_bounds=False):
    gs, indexes, (width_ratios, height_ratios) = get_gridspec(sequence, 1)
    data_index, left_seq_index, bottom_seq_index, cbar_index = indexes
    fig = plt.figure(figsize=(sum(width_ratios), sum(height_ratios)))
    # Add the sequence labels to the left of the figure
    extra_rowcol = bool(lower_kKM_matrix.shape[0] > len(sequence_labels))
    left_sequence_ax, bottom_sequence_ax = add_sequence_labels(fig, gs[left_seq_index],
                                                               gs[bottom_seq_index], 1,
                                                               sequence_labels, 
                                                               extra_rowcol=extra_rowcol)
    left_sequence_ax.set_ylabel("Stop", fontsize=fontsize*2)
    bottom_sequence_ax.set_xlabel("Start", fontsize=fontsize*2)
    # Add data to the main part of the figure
    ms = add_data(fig, gs[data_index], lower_kKM_matrix, upper_kKM_matrix, cmap=cmap,
                  grid_line_spacing=1, normalize=normalize, force_full_bounds=force_full_bounds)
    # Add a color bar to the right side to quantify the colors in the main figure
    add_colorbar(fig, gs[cbar_index], ms, fontsize)


def plot_2d_insertions(sequence, sequence_labels, lower_kKM_matrix, upper_kKM_matrix=None, fontsize=18, cmap='viridis', normalize=False, force_full_bounds=False):
    dimension = 4
    gs, indexes, (width_ratios, height_ratios) = get_gridspec(sequence, dimension)
    data_index, left_seq_index, bottom_seq_index, left_color_index, bottom_color_index, cbar_index = indexes
    fig = plt.figure(figsize=(sum(width_ratios) / 3, sum(height_ratios) / 3))
    # Add sequence labels to left and bottom
    extra_rowcol = bool(lower_kKM_matrix.shape[0] / 4 > len(sequence_labels))
    add_sequence_labels(fig, gs[left_seq_index], gs[bottom_seq_index], dimension, sequence_labels,
                       extra_rowcol=extra_rowcol)
    # Add the color bars to the left and bottom of the figure to indicate which base was inserted
    if extra_rowcol:
        insertion_bases = 'ACGT' * (len(sequence) + 1)
    else:
        insertion_bases = 'ACGT' * len(sequence)
    add_color_axes(fig, gs[left_color_index], gs[bottom_color_index], insertion_bases)
    # Add data to the main part of the figure
    ms = add_data(fig, gs[data_index], lower_kKM_matrix, upper_kKM_matrix, cmap=cmap, show_base_legend=True, grid_line_spacing=dimension, normalize=normalize, force_full_bounds=force_full_bounds)
    # Add a color bar to the right side to quantify the colors in the main figure
    add_colorbar(fig, gs[cbar_index], ms, fontsize)
...
def get_gridspec(sequence, dimension):
    if dimension > 1:
        width_ratios = [.5, 1.0, len(sequence) * dimension, 3.0]
        height_ratios = [len(sequence) * dimension, 1.0, .5]
...
def add_colorbar(fig, colorbar_grid, ms, fontsize, label='$\log_{10}$ Cleavage Rate ($s^{-1}$)'):
...
def add_data(fig, data_grid, lower_kKM_matrix, upper_kKM_matrix, normalize=False, cmap='viridis', show_base_legend=False, grid_line_spacing=None, force_full_bounds=True):
    """
    ...
    data_ax.set_facecolor(0.87 * np.array([1, 1, 1]))
    if show_base_legend:
    ...
    if upper_kKM_matrix is None:
        largest_magnitude = np.nanmax(lower_kKM_matrix)
        if not normalize and force_full_bounds:
            vmin, vmax = -largest_magnitude, largest_magnitude
        elif not normalize:
            vmin = np.nanmin(lower_kKM_matrix)
            vmax = np.nanmax(lower_kKM_matrix)
        else:
            vmin, vmax = -1, 1
        ms = data_ax.matshow(lower_kKM_matrix, cmap=cmap, vmin=vmin, vmax=vmax)
    else:
        ...
        if not normalize:
            ...
            if force_full_bounds:
                ...
                largest_magnitude = max(np.nanmax(np.abs(upper_kKM_matrix)), np.nanmax(np.abs(lower_kKM_matrix)))
                vmin, vmax = -largest_magnitude, largest_magnitude
        ms = data_ax.matshow(sum_nan_arrays(upper_kKM_matrix, lower_kKM_matrix), cmap=cmap, vmin=vmin, vmax=vmax)
    data_ax.set_yticks([])
    data_ax.set_xticks([])
    if grid_line_spacing is not None:
        xlim = data_ax.get_xlim()
        ylim = data_ax.get_ylim()
        for i in np.arange(-.5, lower_kKM_matrix.shape[0]-grid_line_spacing, grid_line_spacing):
...
def add_sequence_labels(fig, left_grid, bottom_grid, dimension, sequence_labels,
                        positions_are_merged=False, extra_rowcol=False):
    # Add the sequence labels to the left of the figure
    ...
    if not extra_rowcol:
        ...
        left_sequence_ax.set_yticks([dimension * x + dimension for x in range(len(sequence_labels))])
        left_sequence_ax.set_ylim([0, (len(sequence_labels) + 1) * dimension])
    ...
    if not extra_rowcol:
        ...
        bottom_sequence_ax.set_xticks([x * dimension + dimension for x in range(len(sequence_labels))])
        bottom_sequence_ax.set_xlim([0, (len(sequence_labels) + 1) * dimension])
    ...
    bottom_sequence_ax.patch.set_alpha(0.0)  # make weird white rectangles go away
